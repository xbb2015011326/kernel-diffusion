...
import scipy
import math
def infer(x, y, mode, iter_size=20):
    """
    ...
    #y = np.linspace(0, 100, N) #5*np.sin(np.pi/15*x)*np.exp(-x/50)
    M = 14  #label_num
    x_new = np.linspace(0, 14, M)
    # initialize and optimize kernel
    if mode == 'rbf':
        initial_param = [2, 2, 2]
        jump_limit = [[0.01, 10], [0.01, 10], [0.01, 10]]
        kernel = RbfKernel(initial_param, jump_limit)
    elif mode == 'dnn':
        initial_param = [1, 2]
        jump_limit = [[0.01, 10], [0.01, 10]]
    ...
    k00_inv = np.linalg.pinv(k00+1e-6)
    sigma2_y = 0.1
    for m in range(M):
...
def log_likelihood(x: np.ndarray, y: np.ndarray, kernel: 'function'):  # noqa: F821
    k00 = calc_kernel_matrix(x, kernel)
    print(k00.shape)
    k00_inv = k00#np.linalg.pinv(math.exp(k00)+1e-6)
...
if __name__ == '__main__':
    ...
    # parser.add_argument('-x', type = float, default = y1[0], help = 'inputs')
    # parser.add_argument('-y',type = float, default = y2[0], help = 'outputs' )
    args = parser.parse_args()
    assert args.mode in ['dnn', 'rbf'], f'{args.mode} mode is not implemented. please choose from "dnn" or "rbf"'
    infer(y1[0], y2[0], args.mode, args.iter_size)
